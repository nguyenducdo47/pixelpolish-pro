...
<head>
    ...
    @php
        ...
        $richProjects = (bool) data_get($portfolio, 'display.rich_cv_projects', true);
        $showTechStack = (bool) data_get($portfolio, 'display.show_tech_stack', true);
        ...
        $makeIcons = function (string $strokeColor, string $fillMuted = '') use ($muted) {
            $svgIcon = function (string $pathD, bool $filled = false) use ($strokeColor, $fillMuted, $muted) {
                $attr = $filled
                    ? 'fill="'.($fillMuted ?: $muted).'"'
        ...
        $icons = $makeIcons($muted);
        $sidebarIcons = $makeIcons('#ffffff', '#ffffff');

        $pdfLayoutClass = match ($layout) {
            'sidebar' => 'cv-pdf-sidebar',
            'classic' => 'cv-pdf-classic',
            default => 'cv-pdf-modern',
        };

        $forPdf = true;
    @endphp
    <style>
        @include('cv.partials.pdf-styles')
    </style>
</head>
<body class="{{ $pdfLayoutClass }}">
    @if($layout === 'sidebar')
        <div class="cv-sheet cv-sidebar">
            <div class="cv-sidebar-layout">
                <div class="cv-sidebar-panel">
                    ...
                    @include('cv.partials.main', [
                        'portfolio' => $portfolio,
                        'profile' => $profile,
                        'settings' => $settings,
                        'only' => 'education',
                        'detailedEducation' => true,
                        'sidebarSection' => true,
                    ])
                    @include('cv.partials.side', [
                        'portfolio' => $portfolio,
                        'profile' => $profile,
                        'settings' => $settings,
                        'only' => 'skills',
    ...
    @else
        <div class="cv-sheet cv-{{ $layout }}">
            @include('cv.partials.header', compact('profile', 'settings', 'icons', 'github', 'stripProtocol', 'layout'))

            @include('cv.partials.main', ['portfolio' => $portfolio, 'profile' => $profile, 'settings' => $settings, 'only' => 'about', 'classicEntries' => $layout === 'classic'])
            @include('cv.partials.main', ['portfolio' => $portfolio, 'profile' => $profile, 'settings' => $settings, 'only' => 'education', 'classicEntries' => $layout === 'classic'])
            @include('cv.partials.side', ['portfolio' => $portfolio, 'profile' => $profile, 'settings' => $settings, 'only' => 'skills', 'classicEntries' => $layout === 'classic'])
            @include('cv.partials.main', [
                'portfolio' => $portfolio,
                'profile' => $profile,
                'settings' => $settings,
                ...
                'richProjects' => $richProjects,
                'showTechStack' => $showTechStack,
                'modernTimeline' => $layout === 'modern',
                'classicEntries' => $layout === 'classic',
            ])
            @include('cv.partials.side', ['portfolio' => $portfolio, 'profile' => $profile, 'settings' => $settings, 'only' => 'languages', 'classicEntries' => $layout === 'classic'])
            @include('cv.partials.main', ['portfolio' => $portfolio, 'profile' => $profile, 'settings' => $settings, 'only' => 'principles', 'classicEntries' => $layout === 'classic'])
        </div>
    @endif
</body>
</html>
